...
# ===========================
COMMANDS = {
    "deploy globalsoft": f"sshpass -p '{SSH_PASSWORD}' ssh -o StrictHostKeyChecking=no {SSH_USER}@{SSH_HOST} '{DEPLOY_SCRIPT_PATH}'",
    "restart globalsoft": f"sshpass -p '{SSH_PASSWORD}' ssh -o StrictHostKeyChecking=no {SSH_USER}@{SSH_HOST} 'docker restart globalsoft_web_prod'",
    "docker ps": f"sshpass -p '{SSH_PASSWORD}' ssh -o StrictHostKeyChecking=no {SSH_USER}@{SSH_HOST} 'docker ps'",
    "logs globalsoft": f"sshpass -p '{SSH_PASSWORD}' ssh -o StrictHostKeyChecking=no {SSH_USER}@{SSH_HOST} 'docker logs globalsoft_web_prod --tail 20'",
    "server status": f"sshpass -p '{SSH_PASSWORD}' ssh -o StrictHostKeyChecking=no {SSH_USER}@{SSH_HOST} 'uptime'"
}
